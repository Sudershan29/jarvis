...
func (uq *UserQuery) QueryPreference() *PreferenceQuery {
	query := (&PreferenceClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithPreference(opts ...func(*PreferenceQuery)) *UserQuery {
	query := (&PreferenceClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withPreference = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withPreference; query != nil {
		if err := uq.loadPreference(ctx, query, nodes, nil,
			func(n *User, e *Preference) { n.Edges.Preference = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadPreference(ctx context.Context, query *PreferenceQuery, nodes []*User, init func(*User), assign func(*User, *Preference)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	for _, n := range neighbors {
		fk := n.user_preference
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_preference" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_preference" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
