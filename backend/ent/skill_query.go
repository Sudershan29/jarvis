...
func (sq *SkillQuery) QueryCategories() *CategoryQuery {
...
func (sq *SkillQuery) WithCategories(opts ...func(*CategoryQuery)) *SkillQuery {
	query := (&CategoryClient{config: sq.config}).Query()
...
func (sq *SkillQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Skill, error) {
	...
	if query := sq.withCategories; query != nil {
		if err := sq.loadCategories(ctx, query, nodes,
			func(n *Skill) { n.Edges.Categories = []*Category{} },
			func(n *Skill, e *Category) { n.Edges.Categories = append(n.Edges.Categories, e) }); err != nil {
			return nil, err
		}
	}
...
func (sq *SkillQuery) loadCategories(ctx context.Context, query *CategoryQuery, nodes []*Skill, init func(*Skill), assign func(*Skill, *Category)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(skill.CategoriesTable)
		s.Join(joinT).On(s.C(category.FieldID), joinT.C(skill.CategoriesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(skill.CategoriesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(skill.CategoriesPrimaryKey[1]))
