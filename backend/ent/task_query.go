...
func (tq *TaskQuery) QueryCategories() *CategoryQuery {
...
func (tq *TaskQuery) QueryTimePreferences() *TimePreferenceQuery {
	query := (&TimePreferenceClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TaskQuery) WithCategories(opts ...func(*CategoryQuery)) *TaskQuery {
	query := (&CategoryClient{config: tq.config}).Query()
...
func (tq *TaskQuery) WithTimePreferences(opts ...func(*TimePreferenceQuery)) *TaskQuery {
	query := (&TimePreferenceClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := tq.withCategories; query != nil {
		if err := tq.loadCategories(ctx, query, nodes,
			func(n *Task) { n.Edges.Categories = []*Category{} },
			func(n *Task, e *Category) { n.Edges.Categories = append(n.Edges.Categories, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := tq.withTimePreferences; query != nil {
...
func (tq *TaskQuery) loadCategories(ctx context.Context, query *CategoryQuery, nodes []*Task, init func(*Task), assign func(*Task, *Category)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(task.CategoriesTable)
		s.Join(joinT).On(s.C(category.FieldID), joinT.C(task.CategoriesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(task.CategoriesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(task.CategoriesPrimaryKey[1]))
...
func (tq *TaskQuery) loadTimePreferences(ctx context.Context, query *TimePreferenceQuery, nodes []*Task, init func(*Task), assign func(*Task, *TimePreference)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Task)
	nids := make(map[int]map[*Task]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(task.TimePreferencesTable)
		s.Join(joinT).On(s.C(timepreference.FieldID), joinT.C(task.TimePreferencesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(task.TimePreferencesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(task.TimePreferencesPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
