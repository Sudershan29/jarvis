...
func (mq *MeetingQuery) QueryUser() *UserQuery {
...
func (mq *MeetingQuery) WithUser(opts ...func(*UserQuery)) *MeetingQuery {
	query := (&UserClient{config: mq.config}).Query()
...
func (mq *MeetingQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Meeting, error) {
	...
	if query := mq.withUser; query != nil {
		if err := mq.loadUser(ctx, query, nodes, nil,
			func(n *Meeting, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MeetingQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Meeting, init func(*Meeting), assign func(*Meeting, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Meeting)
	for i := range nodes {
		if nodes[i].user_meetings == nil {
			continue
		}
		fk := *nodes[i].user_meetings
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_meetings" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
